    ...
    Alpine.data('toastContainer', () => ({
        ...
        icon(type) {
            const icons = {
                ...
                error:   "<svg class='w-5 h-5' xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='currentColor'><path fill-rule='evenodd' d='M12 2.25c-5.385 0-9.75 4.365-9.75 9.75s9.75-4.365 9.75-9.75S17.385 2.25 12 2.25Zm-1.72 6.97a.75.75 0 1 0-1.06 1.06L10.94 12l-1.72 1.72a.75.75 0 1 0 1.06 1.06L12 13.06l1.72 1.72a.75.75 0 1 0 1.06-1.06L13.06 12l1.72-1.72a.75.75 0 1 0-1.06-1.06L12 10.94l-1.72-1.72Z' clip-rule='evenodd'/></svg>",
...
window.showToast = function(message, type = 'success', duration = 4000) {
    ...
    if (container && container._x_dataStack && container._x_dataStack[0]) {
        container._x_dataStack[0].add(message, type, duration);
    }
};

window.showConfirm = function(detail) {
    const dialog = document.querySelector('[x-data*="confirmDialog"]');
    if (dialog && dialog._x_dataStack && dialog._x_dataStack[0]) {
        dialog._x_dataStack[0].show(detail);
    }
};